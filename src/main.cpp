...
#if defined(NDEBUG)
# error "Mattcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x139567bd5a959400d112a662f4768ee5bb77fa17fea14e1b310461fb73848ee8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mattcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mattcoin Signed Message:\n";
...
{
    // Mattcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Mattcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Mattcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Mattcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mattcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mattcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3040c252ef151298c919eb46fa257a6fb2639ef3ed88aa389bac7cf690448331");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "OPLYNX app goes live for CNRL";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0469a6ae797d55b5fa3fc87d2581a667a5967f5e81f35b40393b47aa6ae1778cc5e3338e1b6d5ecddcc57f343511dcd54e01706d8310d8d1a5e7e3da48f3720460") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516660471;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086379202;

        if (fTestNet)
        {
            block.nTime    = 1516660455;
            block.nNonce   = 386187438;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5ac999220de94cc97a561121a708d5bbbbde76594d956f8390b4ef521640837f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xbb, 0xd2 }; // Mattcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MattcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MattcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MattcoinMiner(CWallet *pwallet)
{
    printf("MattcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mattcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MattcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MattcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MattcoinMiner, pwallet));
}
